...
{
    /// <summary>
    /// Tracks elapsed time since the MountainSide_Earthquake scene started.
    /// Call FinalizeAndGetData() when the game ends to stop the timer and retrieve session data.
    /// </summary>
    public class MountainsideAssessmentTracker : MonoBehaviour
    {
        public static MountainsideAssessmentTracker Instance { get; private set; }
        ...
        public MountainsideAssessmentData FinalizeAndGetData()
        {
            _isTracking = false;
            return new MountainsideAssessmentData(_elapsedSeconds);
        }
    }

    public sealed class MountainsideAssessmentData
    {
        ...
        public MountainsideAssessmentData(float totalSeconds)
        {
        ...
        {
            if (TotalSeconds <= OutstandingThreshold) return new RewardData(3, "Mountain Conqueror",  BadgeTier.Gold);
            if (TotalSeconds <= GoodThreshold)        return new RewardData(3, "Swift Evacuator",     BadgeTier.Gold);
