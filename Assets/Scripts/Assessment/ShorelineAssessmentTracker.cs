...
{
    /// <summary>
    /// Tracks elapsed time since the Shoreline_Earthquake scene started.
    /// Call FinalizeAndGetData() when the game ends to stop the timer and retrieve session data.
    /// </summary>
    public class ShorelineAssessmentTracker : MonoBehaviour
    {
        public static ShorelineAssessmentTracker Instance { get; private set; }
        ...
        public ShorelineAssessmentData FinalizeAndGetData()
        {
            _isTracking = false;
            return new ShorelineAssessmentData(_elapsedSeconds);
        }
    }

    public sealed class ShorelineAssessmentData
    {
        ...
        public ShorelineAssessmentData(float totalSeconds)
        {
