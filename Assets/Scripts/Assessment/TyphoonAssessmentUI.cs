...
{
    /// <summary>
    /// Drives the end-game Assessment Panel for the Typhoon scenario across two slides:
    /// Slide 1 — Stats (time, rating, learned content) with a Next button.
    /// Slide 2 — Reward (badge name, stars, motivational message) with Back to Menu.
    ///
    /// Hook ShowAssessmentAfterDialogue() to the final objective's CompleteEvent in the Inspector,
    /// or use a trigger that calls ShowAssessment() directly after the ending dialogue plays.
    /// </summary>
    public class TyphoonAssessmentUI : MonoBehaviour
    {
        public static TyphoonAssessmentUI Instance { get; private set; }
        ...
        [Header("Stats Slide (leave null for single-slide mode)")]
        ...
        [Header("Reward Slide (leave null for single-slide mode)")]
        ...
        [SerializeField] private int finalObjectiveID = 6;
        ...
        private TyphoonAssessmentData _lastData;

        private bool TwoSlideMode => statsSlide != null && rewardSlide != null;

        private const string LearnedContent =
            "• When a typhoon warning is raised, stay indoors and away from windows and doors.\n\n" +
            "• Prepare an emergency kit with food, water, medicine, and important documents.\n\n" +
            "• Turn off electrical breakers before evacuating to prevent fire and electrocution.\n\n" +
            "• Follow official evacuation orders immediately — never wait until the last moment.\n\n" +
            "• Know your community's designated evacuation center and the safest route to reach it.";
        ...
        /// <summary>
        /// Hook this to the final objective's CompleteEvent.
        ...
        private IEnumerator DelayedAssessment()
        {
            ...
            if (ObjectiveManager.HasReference)
                ObjectiveManager.Instance.CompleteObjective(finalObjectiveID);
        ...
        {
            if (TyphoonAssessmentTracker.Instance == null)
            {
                Debug.LogError("[TyphoonAssessmentUI] TyphoonAssessmentTracker not found in scene.");
                return;
            }

            _lastData = TyphoonAssessmentTracker.Instance.FinalizeAndGetData();
        ...
        private void PopulateStatsSlide(TyphoonAssessmentData data)
        {
        ...
        /// <summary>Called by the Next button — cross-fades to the reward slide.</summary>
        ...
        private void PopulateRewardSlide(TyphoonAssessmentData data)
        {
            TyphoonAssessmentData.RewardData reward = data.GetReward();
            ...
            if (badgeImage != null)
            {
                ...
                {
                    TyphoonAssessmentData.BadgeTier.Gold   => goldBadgeSprite,
                    TyphoonAssessmentData.BadgeTier.Silver => silverBadgeSprite,
                    _                                      => bronzeBadgeSprite
                };
            }
            ...
            {
                Debug.LogWarning("[TyphoonAssessmentUI] rewardStarImages must contain exactly 3 elements.");
                return;
            }
            ...
            for (int i = 0; i < rewardStarImages.Length; i++)
            {
                ...
                {
                    Debug.LogWarning($"[TyphoonAssessmentUI] rewardStarImages[{i}] is null.");
                    continue;
                }
        ...
        private static string GetMotivationalMessage(int stars)
        {
            return stars switch
            {
                3 => "Excellent work! You stayed calm, prepared well, and evacuated safely.",
                2 => "Good job! Keep practicing to sharpen your typhoon readiness.",
                _ => "Keep going! Every drill brings you one step closer to being truly prepared."
            };
        }
